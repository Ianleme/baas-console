...
describe('DemoService', () => {
  ...
  test.each([
    ...
    [
      ...
      (token: ReturnType<DemoService['issueSession']>) => expect(token.principal.demo).toBe(true)
    ],
    [
      'has a short expiry',
      (token: ReturnType<DemoService['issueSession']>) =>
  ...
  ])('%s', (_name, assertion) => assertion(service.issueSession(now)));

  test.each([
    [
      'accepts a valid session',
      (token: string) => expect(service.verifySession(token, now)).toMatchObject({ demo: true })
    ],
    [
      'rejects a tampered session',
      (token: string) => expect(service.verifySession(`${token}x`, now)).toBeUndefined()
    ],
    [
      'rejects an expired session',
      (token: string) => expect(service.verifySession(token, now + 900_000)).toBeUndefined()
    ]
  ])('%s', (_name, assertion) => assertion(service.issueSession(now).accessToken));
