    ...
    <section className="reconciliation-page space-y-6" aria-labelledby="reconciliation-title">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 id="reconciliation-title" className="text-3xl font-extrabold text-slate-900 mt-1">Reconciliação</h1>
          <p className="text-slate-500 text-sm mt-1">Acompanhe resultados pendentes e divergências sem alterar estados manualmente.</p>
        </div>
      ...
      {notice && (
        <div aria-live="polite" className="reconciliation-notice bg-emerald-50 border border-emerald-200 text-emerald-800 p-3 rounded-lg text-sm font-semibold">
          {notice}
        </div>
      )}

      {state === 'loading' && <p role="status" className="text-slate-500 p-4">Carregando reconciliações…</p>}
      {state === 'error' && <p role="alert" className="text-red-600 p-4">Não foi possível carregar as reconciliações.</p>}
            ...
            <Card key={item.id} className="reconciliation-item p-4">
              <CardContent className="p-0 flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="reconciliation-state flex flex-col items-end gap-1">
                  <span className="text-xs text-slate-500 font-medium">{item.status.replaceAll('_', ' ')}</span>
...
function ClassificationBadge({ classification }: { classification: ReconciliationItem['classification'] }) {
