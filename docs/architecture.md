...
A API é um serviço NestJS respaldado por MySQL; a aplicação web Vite chama a API; a integração de e-mail usa a API do Brevo quando configurada. O Compose de produção coloca MySQL em uma rede interna e expõe HTTP somente por meio da borda HTTPS. As migrations são explícitas e executadas antes de a API ficar pronta.
...
O serviço recebe `BREVO_API_KEY`, `BREVO_SENDER_EMAIL` e `BREVO_SENDER_NAME` por variáveis de ambiente, sem segredos embutidos no Compose. O remetente deve ser verificado no Brevo; sem uma chave válida, a entrega de e-mail não é evidência deste setup.
